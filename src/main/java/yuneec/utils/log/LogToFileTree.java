        ...
        dequeueThread = new Thread(new Runnable() {
            public void run() {
                if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                    ...
                    File dir = new File(rootDir, "/YuneecPilot/logcat/");
                    if (!dir.exists()) {
                        if (!dir.mkdirs()) {
                            return;
                        }
                    ...
                    String fileName = String.format("version_%s_%s.log", version, FILE_DATE_FORMAT.format(new Date()));
